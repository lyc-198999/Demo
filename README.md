...
4. 初始方向先用 5 个采样点做线性趋势判断；趋势显著下降或后续未出现有效上升时，会丢弃单侧探测数据并切换为反方向扫描。
5. 判峰逻辑要求最高点前出现有效上升，并且最高点后出现 3 个有效低于峰值的过峰采样。
6. 已经接近焦面并出现过峰趋势后自动切换为小步长移动，并保持小步长直到峰值确认。
7. 峰值确认后优先使用高斯拟合中心作为目标并一次性回焦；拟合中心异常时退回采样峰值。
8. 最终回焦后同时复核目标位置和当前清晰度；位置未到目标会继续修正，清晰度未达到采样峰值比例时继续小步长复核。
